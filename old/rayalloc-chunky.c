...
typedef struct array_free {
	...
	struct array_free **reference[]; // null-terminated array of back-references to any pointers that point to this array, eg fastbins
} array_fre_t;
...
void debug_overview(void) {
	for (int dll=0; dll<3; dll++) {
		chunk_t *c = ((chunk_t*[3]){chkdll_free, chkdll_used, chkdll_full})[dll];
		while (c) {
			printf("\e[1m\e35m[\e[22m%5s", 
				c->free?"free":"used");
			// incomplete
		}
	}
